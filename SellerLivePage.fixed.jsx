...
export default function SellerLivePage() {
  ...
  // Improved camera access with fallback constraints
  const getMediaStream = async () => {
    if (!navigator.mediaDevices?.getUserMedia) {
      throw new Error(
        'Camera API is unavailable. Use HTTPS or localhost, and ensure you\'re using a modern browser.'
      );
    }
    ...
    if (!isSecureContext && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
      throw new Error(
        'Camera access requires HTTPS or localhost. Your site must be served securely.'
      );
    }

    let stream = null;

    // Try to get video + audio together first (most browsers like this)
    try {
      console.log('[live] Attempting to get camera + microphone together...');
      ...
      console.log('[live] Successfully got camera + microphone');
      return stream;
    } catch (err) {
      console.warn('[live] Combined video+audio request failed, trying video only:', err.name);
    }

    // Fallback: get video only (required)
    try {
      console.log('[live] Attempting to get camera only...');
      stream = await navigator.mediaDevices.getUserMedia({
        video: {
          width: { ideal: 1280 },
          height: { ideal: 720 },
      ...
      console.log('[live] Successfully got camera (video only)');

      // Try to add audio separately as fallback
      try {
        console.log('[live] Attempting to add audio track...');
        ...
        audioStream.getAudioTracks().forEach((track) => {
          ...
          console.log('[live] Audio track added');
        });
      } catch (audioErr) {
        console.warn('[live] Audio not available, continuing with video only:', audioErr.name);
      }

      return stream;
    } catch (videoErr) {
      console.error('[live] Video request failed:', videoErr.name, videoErr.message);

      // Provide specific error messages based on error type
      if (videoErr.name === 'NotAllowedError') {
        throw new Error(
          'Camera permission denied. Please check browser settings and allow camera access, then reload.'
        );
        ...
        console.log('[live] Retrying with minimal constraints...');
        try {
          stream = await navigator.mediaDevices.getUserMedia({
            video: true,
            audio: false,
          });
          console.log('[live] Got camera with minimal constraints');
          return stream;
        } catch (finalErr) {
          throw new Error(
            'Camera device does not meet requirements. Try a different camera or browser.'
          );
        }
      }

      throw new Error(`Camera access failed: ${videoErr.name}. Please try again.`);
    }
  };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      // NOW get media with improved error handling
      let stream;
      try {
        stream = await getMediaStream();
      } catch (e) {
        console.error('[live] Media stream error:', e);
      ...
      if (localVideoRef.current) {
        ...
        try {
          ...
          console.log('[live] Local video playback started');
        } catch (playErr) {
