...
const uploadStorefrontAssets = upload.fields([
  { name: "logo", maxCount: 1 },
  { name: "banner", maxCount: 1 },
]);

function normalizeSellerIdPart(v) {
...
/** Resolves seller id from query, header, or multipart/json body (body is set after multer/json). */
function requireSellerId(req) {
  return (
    normalizeSellerIdPart(req.query.sellerId) ||
    normalizeSellerIdPart(req.headers["x-seller-id"]) ||
    normalizeSellerIdPart(req.body?.sellerId) ||
    null
  );
}
...
/** Public: list storefront summaries */
router.get("/storefronts", async (req, res) => {
  try {
    ...
    const storefronts = await SellerStorefrontModel.find(filter)
      .sort({ updatedAt: -1 })
      .select(
        "storeName slug logoUrl bannerUrl description categoryTags updatedAt createdAt"
      )
      .lean();
...
/** Seller: current user's storefront + products */
router.get("/my-storefront", async (req, res) => {
  try {
    ...
    if (!storefront) {
      res.json({ storefront: null, products: [] });
      return;
    }
...
/** Public: one storefront + products */
...
/** Seller: create storefront (one per seller) */
router.post("/storefronts", uploadStorefrontAssets, async (req, res) => {
  try {
    const sellerId = requireSellerId(req);
    if (!sellerId) {
      res
    ...
    if (existing) {
      res.status(409).json({
        error: "You already have a storefront. Use PATCH to update it.",
        storefrontId: String(existing._id),
      });
      return;
    }
    ...
    const doc = await SellerStorefrontModel.create({
      sellerId,
      storeName,
      slug,
      description,
...
/** Seller: update storefront */
router.patch("/storefronts/:id", uploadStorefrontAssets, async (req, res) => {
  try {
    const sellerId = requireSellerId(req);
    if (!sellerId) {
      res
    ...
    if (req.body.storeName != null) {
      ...
      if (name && name !== storefront.storeName) {
        storefront.storeName = name;
        storefront.slug = await uniqueSlugFromName(name, storefront._id);
      }
    }
    if (req.body.description != null) {
      storefront.description = String(req.body.description).trim();
    }
    if (req.body.categoryTags != null) {
      storefront.categoryTags = parseStringArray(req.body.categoryTags);
    }

    const files = req.files;
    if (files?.logo?.[0]) {
      storefront.logoUrl = `/uploads/${files.logo[0].filename}`;
    }
    if (files?.banner?.[0]) {
      storefront.bannerUrl = `/uploads/${files.banner[0].filename}`;
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!mongoose.Types.ObjectId.isValid(req.params.storefrontId)) {
        res.status(400).json({ error: "Invalid storefront id." });
        return;
      }
      const storefront = await SellerStorefrontModel.findOne({
        _id: req.params.storefrontId,
        sellerId,
      });
      if (!storefront) {
        res.status(404).json({ error: "Storefront not found." });
      ...
      if (!name || Number.isNaN(price) || price < 0) {
        res
          .status(400)
          .json({ error: "name and a valid non-negative price are required." });
        return;
      }
      let imageUrl;
      if (req.file) {
      ...
      const doc = await StorefrontProductModel.create({
        sellerId,
        storefrontId: storefront._id,
        name,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const product = await StorefrontProductModel.findOne({
...
router.delete("/products/:productId", async (req, res) => {
  try {
    ...
    const result = await StorefrontProductModel.deleteOne({
      _id: req.params.productId,
      sellerId,
    });
